...
#if UE_5_6_OR_LATER
	auto Result = FPlatformFileManager::Get().GetPlatformFile().OpenMappedEx(*InInputPathName);
	if (Result.IsValid() && Result.HasValue())
	{
		MappedHandle = Result.StealValue().Release();
	}
#else
	MappedHandle = FPlatformFileManager::Get().GetPlatformFile().OpenMapped(*InInputPathName);
#endif
	...
	const FWwisePlatformDataStructure& InPlatformData) const
{
	...
	else if (Media->Location == EWwiseMetadataMediaLocation::OtherBank)
	{
		...
		if (OtherMedia->Location != EWwiseMetadataMediaLocation::Memory)
		{
			FWwiseMediaCookedData MediaCookedData;
