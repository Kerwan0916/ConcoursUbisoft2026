	...
	const FName& InStat)
{
	...
	IMappedFileHandle* Handle = nullptr;
#if UE_5_6_OR_LATER
	auto Result = PlatformFile.OpenMappedEx(*InFilePathname);
	if (Result.HasValue())
	{
		Handle = Result.StealValue().Get();
	}
#else
	Handle = PlatformFile.OpenMapped(*InFilePathname);
#endif
	if (UNLIKELY(!Handle))
	{
