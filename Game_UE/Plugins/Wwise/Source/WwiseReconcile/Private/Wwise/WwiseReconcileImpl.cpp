...
void FWwiseReconcileImpl::GetAllWwiseRefs()
{
	...
	for (const auto& WwiseRef : DataStructure.GetCurrentPlatformData()->Guids)
	{
		if (WwiseRef.Value.GetType() != EWwiseRefType::SoundBank)
		{
			...
			ShortIdToWwiseRef.Add(WwiseRef.Value.GetId(), { &WwiseRef.Value });
		}
	}
}
...
bool FWwiseReconcileImpl::ShouldMove(const FWwiseAnyRef& Ref, FAssetData InAssetPath, FString& OutNewAssetPath) const
{
...
FString FWwiseReconcileImpl::GetAssetPackagePath(const FWwiseAnyRef& WwiseRef) const
{
