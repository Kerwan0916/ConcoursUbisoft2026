...
public:
	...
	virtual FString GetAssetPackagePath(const FWwiseAnyRef& WwiseRef) const = 0;
	...
	virtual bool ShouldMove(const FWwiseAnyRef& Ref, FAssetData InAssetPath, FString& OutNewAssetPath) const = 0;

	bool UAssetExists(const FWwiseAnyRef* WwiseRef) const;
	bool IsPathTooLong(const FWwiseAnyRef* WwiseRef) const;
